...
def train(args):
    ...
    torch.cuda.set_device(int(args["device"][0]))  # 将默认gpu设为 args["device"]
...
def _train(args):
    ...
    if len(cnn_matrix)>0:
        ...
        max_wid = max(task + 1, len(cnn_matrix[-1]))  # 原始代码会超长
        np_acctable = np.zeros([task + 1, max_wid])
    ...
    if len(nme_matrix)>0:
        max_wid = max(task + 1, len(nme_matrix[-1]))  # 原始代码会超长
        np_acctable = np.zeros([task + 1, max_wid])
