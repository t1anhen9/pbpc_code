...
class ODEFunc(nn.Module):
    def __init__(self, in_features):
        ...
        self.fc = nn.Linear(in_features, in_features)  # 仅一层

    def forward(self, t, y):
        return self.fc(y)  # 直接输出
...
class BaseNet(nn.Module):
    def __init__(self, args, pretrained):
        ...
        # 测试引入常微分方程
        # self.ode_func = ODEFunc(in_features=512)
        
        if 'ODE' in args:
            self.ode = args['ODE']
        else:
            self.ode = False  # 或者其他默认值
    ...
    def extract_vector(self, x):
        if self.ode:
            ...
            ode_output = odeint(self.ode_func, features.unsqueeze(0), t)  # ODE求解
            
            return ode_output[1].squeeze(0)
        else:
